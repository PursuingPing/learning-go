...
func (this *HelloZinxRouter) Handle(request ziface.IRequest) {
	...
	err := request.GetConnection().SendMsg(201, []byte("Hello...Zinx...1.0"))
	if err != nil {
		log.Println(err)
	}
}
...
func DoConnectionBegin(conn ziface.IConnection) {
	...
	err := conn.SendMsg(202, []byte("DoConnectionBegin...Zinx...1.0"))
	if err != nil {
		log.Println(err)
	}
	//给当前连接设置一些属性
	log.Println("====>Set conn Name, Ping")
	conn.SetProperty("Name", "PingPingPIng")
	conn.SetProperty("Home", "www.github.com")
	conn.SetProperty("Sina", "hh")
}
...
func DoConnectionLost(conn ziface.IConnection) {
	...
	if sina, err := conn.GetProperty("Sina"); err == nil {
		log.Println("Sina = ", sina)
	}
}
